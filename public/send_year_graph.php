...
/*
  ...
  ------------------------------
  File    : send_year_graph.php
  Desc.   : return PNG width plotted graph of rankings

  Version : 0.3
  Author  : Matthijs Koot

  History : 12-06-03 - file created
            19-06-03 - now returning PNG w/msg on error
            25-07-03 - chart presentation improved (could be better though)
...
/* get report data */
  ...
  if ($result_rp->EOF)
    ...
    /* get graph data for all keywords (this website, SE and year) */
    $sql_rr="SELECT
               $DB_Reportrules.zt_id,
               $DB_Reportrules.ranking,
               month($DB_Reports.rankdate) AS rankmonth,
               $DB_Keyphrases.keyword
             FROM
               $DB_Reports,
               $DB_Keyphrases,
               $DB_Reportrules
             WHERE
               ...
               AND $DB_Keyphrases.id = $DB_Reportrules.zt_id";
      ...
      while ($o = $result_rr->FetchNextObject()) {
        ...
        $graphdata[$o->ZT_ID][$o->RANKMONTH] = $ranking;
      }
...
$graph->title->Set("$se_title, $rankyear");
...
foreach($graphdata as $phrase1) { 
  ...
  // prevent a 13th month in our plot
  unset($phrase1[12]);
